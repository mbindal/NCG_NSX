...
class Load_Balancer_floating_ip(object):
    def __init__(self, network_id, subnet_mapping, sec_id,quantum):
            self.i=self.j=0;
    ...
    def Create_Loadbalancers(self):
        for individual_pools in mysets.pool['bulk_pool']:
            subnet_id=self.subnet_mapping[individual_pools['pool']['subnet_id']]
            individual_pools['pool']['subnet_id']=subnet_id
            pool=self.quantum.create_pool(individual_pools)['pool']
            for pools in mysets.pool_member[pool['name']]:
            ...
            vip_portid=self.quantum.create_vip(mysets.vip['vips'][self.i])['vip']['port_id']
